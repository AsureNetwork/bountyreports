        ...
        member.weeks.forEach(week => {
            Object.keys(week.campaigns).forEach(campaign => {
                if (week.campaigns[campaign].done) {
                    ...
                    stats.campaignTotals[campaign].count += 1;
                    if (!tokenBasedCampaigns.includes(campaign)) {
                        stats.campaignTotals[campaign].stakes += 5;
                    }
                }
                ...
                stats.byMemberAndCampaign[member.address][campaign].count += 1;
                if (!tokenBasedCampaigns.includes(campaign)) {
                    stats.byMemberAndCampaign[member.address][campaign].stakes += 5;
                }

                stats.byMember[member.address] = stats.byMember[member.address] || {count: 0, stakes: 0};
                stats.byMember[member.address].count += 1;
                if (!tokenBasedCampaigns.includes(campaign)) {
                    stats.byMember[member.address].stakes += 5;
                }
            });
        });
